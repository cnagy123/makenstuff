...
void InitTask(void const * argument)
{
	LOCK_TCPIP_CORE();						// Lock the TCPIP core whenever call-back style api is used outside tcpip thread
	MX_LWIP_Init();							// initialize the tcpip thread and netif
	UNLOCK_TCPIP_CORE();					// Unlock the core when done

  /* Initialize the global app struct  */
  if(osSemaphoreWait(appSemHandle,0)==osOK){	// Protect global changes with the semaphore
  	app.connection_type = 0;					// set connection type to STATIC (LED 3 off)
  	app.ip_acquire = 1;							// set the need to aquire an IP address
  	app.state = IDLE;							// set the initial state
  	app.link = &link_arg;						// set the global link argument
  	osSemaphoreRelease(appSemHandle);			// Release the semaphore when done
  }

  /* definition and creation of switchIP */
  osThreadDef(switchIP, SwitchIP, osPriorityIdle, 0, 256);
  switchIPHandle = osThreadCreate(osThread(switchIP), &app);	// pass the app struct via pointer to the task
