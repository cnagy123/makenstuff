...
int main(void)
 {
  ...
  /* Create the mutex(es) */
  /* definition and creation of lidarMutex */
  osMutexDef(lidarMutex);
  lidarMutexHandle = osMutexCreate(osMutex(lidarMutex));

  /* definition and creation of mainAppMutex */
  osMutexDef(mainAppMutex);
  mainAppMutexHandle = osMutexCreate(osMutex(mainAppMutex));
  ...
  /* definition and creation of appSem */
  ...
  /* Create the queue(s) */
  /* definition and creation of lidarQueue */
  osMessageQDef(lidarQueue, 16, uint16_t);
  lidarQueueHandle = osMessageCreate(osMessageQ(lidarQueue), NULL);

  /* definition and creation of printInfoQueue */
  osMessageQDef(printInfoQueue, 16, uint16_t);
  printInfoQueueHandle = osMessageCreate(osMessageQ(printInfoQueue), NULL);

  /* definition and creation of cmdLineQueue */
  osMessageQDef(cmdLineQueue, 16, uint16_t);
  cmdLineQueueHandle = osMessageCreate(osMessageQ(cmdLineQueue), NULL);
  ...
  /* definition and creation of mainAppTask */
  osThreadDef(mainAppTask, MainAppTask, osPriorityIdle, 0, 128);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(Lidar_GPIO_Port, Lidar_Pin, GPIO_PIN_RESET);
...
/**
* @brief Function implementing the printInfoTask thread.
* @param argument: Not used
* @retval None
*/
